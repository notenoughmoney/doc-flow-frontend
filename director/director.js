...
// инициализация таблицы с расписанием завучей
function initTableWithHeadTeachers() {
  ...
  // полученные доки отфильтруем и возмем только завучей
  var filtered = [];
  for (let i = 0; i < data.length; i++) {
    if (data[i].job === 3 || data[i].job === 4 || data[i].job === 5) {
      filtered.push(data[i]);
    }
  }
